-- Script para corregir el error de columna ambigua "asesor_id"
-- Especifica claramente de qué tabla viene cada columna

-- 1. Eliminar el trigger anterior
...
-- 2. Crear la función corregida con columnas específicas
...
BEGIN
  ...
  IF NEW.estado IN ('aprobada', 'rechazada') AND OLD.estado = 'pendiente' THEN
    ...
    -- Buscar asesor que creó la petición (especificando la tabla)
    SELECT p.asesor_id INTO asesor_id
    FROM peticiones_cambio_carrera p
    WHERE p.id = NEW.peticion_id;
